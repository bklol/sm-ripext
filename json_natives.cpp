...
static cell_t CreateObjectKeys(IPluginContext *pContext, const cell_t *params)
{
	...
	JSONObjectKeys *keys = new JSONObjectKeys(object);

	Handle_t hndlKeys = handlesys->CreateHandle(htJSONObjectKeys, keys, pContext->GetIdentity(), myself->GetIdentity(), NULL);
...
static cell_t GetObjectKey(IPluginContext *pContext, const cell_t *params)
{
	HandleError err;
	HandleSecurity sec(pContext->GetIdentity(), myself->GetIdentity());
...
const sp_nativeinfo_t json_natives[] =
{
	...
	{"JSONObjectKeys.GetKey",			GetObjectKey},
